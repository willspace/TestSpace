...
NEJ.define([
    ...
    'pro/page/contents'
],function(_k,_e,_v,_ut,_mi,_si,_ci,_notify,_j,_ct,_p){
    var _pro;
    ...
    _pro.__init = function(_options){
        ...
        this.__mobileInput = new _mi({data:{placeholder:"请输入你要继续使用的手机号"}});
    ...
    _pro.__doNext = function(){
        ...
        if ( this.__smsInput.data.state==1 ){
            //// doNext
            var _url = '/web/freezappeal/unfreez_acc_e';
            var _data = {
    ...
    _pro.__onWarn = function(_json){
        ...
        if(!!_json){
            var _txt = _ct[_code] || '未知错误，请你稍后再试';
            if(_code == "604" || _code == "603" ||_code == "601" || _code == "605"){
                _txt = _code == "604" ? "账号已存在" : _txt;
